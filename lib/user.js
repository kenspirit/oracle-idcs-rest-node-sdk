...
module.exports = class User {
  ...
  resetPassword(userId, newPassword, bypassNotification = false) {
    const self = this;
    return this.idcs.getToken({
      scope: 'urn:opc:idm:__myscopes__',
      grant_type: 'client_credentials'
      ...
      .then((oracleAppToken) => {
        return new Promise((resolve, reject) => {
          return self.idcs.gateway({
            ...
            url: `${self.idcs.options.oracle.REST_BASE_URL}/UserPasswordChanger/${userId}`,
            body: {
      ...
      .then((oracleAppToken) => {
        return new Promise((resolve, reject) => {
          return self.idcs.gateway({
            ...
            url: `${self.idcs.options.oracle.REST_BASE_URL}/Users?filter=(userName eq "${encodeURIComponent(username)}")`
          }, (err, res) => {
            if (err) {
              return reject(err);
            } else if (res.statusCode === 200) {
      ...
      .then((oracleAppToken) => {
        return new Promise((resolve, reject) => {
          return self.idcs.gateway({
            ...
            url: `${self.idcs.options.oracle.REST_BASE_URL}/Users/${userId}?attributeSets=all`
          }, (err, res) => {
            if (err) {
              return reject(err);
            } else if (res.statusCode === 200) {
